...
class CourseComparison extends Component {
  ...
  handleCourse1Change = (params) => {
    const { course1Uuid, course2Uuid, location, history } = this.props;
  ...
  handleReplaceCourse1 = () => {
    ...
    history.push(`/search?compareWith=${course2Uuid}&replacing=1`);
  };

  handleReplaceCourse2 = () => {
    const { course1Uuid, history } = this.props;
    // Navigate to search with course1 as the comparison target
    history.push(`/search?compareWith=${course1Uuid}&replacing=2`);
  };

  render() {
    const { course1Uuid, course2Uuid, onRemoveComparison, location } = this.props;
